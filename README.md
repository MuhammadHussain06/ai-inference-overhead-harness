...
- **WSL2/Docker Desktop specifically: `verify_cpu_pinning()` can pass while pinning still isn't real.** cgroup-level `cpuset` is honored inside the WSL2 VM, so the requested-vs-live cgroup check reports OK — but the Hyper-V host scheduler can still migrate the underlying virtual CPUs across physical cores independent of that, and this script has no way to observe or catch it. Distinct from the general Docker Desktop caveat above: here the pass/fail signal itself is unreliable, not just the isolation. `run-suite.sh` detects WSL2 (checks `/proc/version`) and records `wsl2_detected` in `run_metadata.json` so any affected `results/` snapshot is traceable after the fact, but this is disclosure, not mitigation — treat concurrency-scan tail-latency claims (E2) as more exposed to this than the baseline decomposition (E1), since scheduling pressure — and therefore migration risk — scales with load.
