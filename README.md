...
- `run-suite.sh`: full baseline + concurrency-scan suite across clean-slate restarts, with per-repetition target/concurrency shuffling, CPU-pin verification, host/toolchain provenance fingerprinting, and a failures log that records why the suite stopped (a single broken cell aborts the whole run rather than being skipped).
...
A small, fast pass through the same pipeline (2 targets, 2 concurrency levels, 2 reps, reduced iteration counts) that catches a structural problem, such as a bad config, a broken tag, or a mislabeled tier, in minutes instead of hours into a real run. It also fires one deliberately over-rate open-loop request to confirm `dropped_iterations` is actually detected.
...
Check `run_failures_log.txt` and `cpu_pin_check_log.txt` afterward, and confirm `table7_openloop_validity_check` shows a nonzero dropped-iterations count for the smoke-openloop cell. A clean pass means the pipeline itself is trustworthy; it says nothing about whether any given rep count is sufficient. Re-run the smoke test after any fix until it passes, then move to the full suite.

`run-suite.sh`'s `TARGETS`, `CONCURRENCY_LEVELS`, `REPS_BASELINE`, `REPS_SCAN`, `BASELINE_ITERATIONS`, and `SCAN_ITERATIONS_PER_VU` are all overridable via `TARGETS_OVERRIDE`, `CONCURRENCY_OVERRIDE`, `REPS_BASELINE_OVERRIDE`, `REPS_SCAN_OVERRIDE`, `BASELINE_ITERATIONS_OVERRIDE`, and `SCAN_ITERATIONS_PER_VU_OVERRIDE` env vars. `run-smoke-test.sh` is a thin wrapper that sets these to a small slice; left unset, `run-suite.sh` behaves exactly as the full suite.
...
The main suite's concurrency scan is closed-loop (see [Limitations](#limitations)). `run-target-openloop.js` runs the same target under a `constant-arrival-rate` executor instead, so requests fire on a fixed schedule regardless of how fast responses come back. This is the standard mitigation for coordinated omission. Run it against the top 1 to 2 concurrency cells only, after the main suite, as a check on whether the closed-loop tail-latency numbers hold up:
...
`RATE` is requests per `TIME_UNIT`. Set it to roughly the throughput the closed-loop cell achieved at that VUS level rather than the VUS count itself. `PRE_ALLOCATED_VUS`/`MAX_VUS` must be generous enough to sustain `RATE` if response times climb. k6 logs a `dropped_iterations` warning if it runs out of headroom; that warning is itself diagnostic, since it means the server can't sustain that arrival rate rather than pointing to a script problem. This script is standalone and manual by design: `run-suite.sh` never invokes it, and it doesn't replace the main suite.
